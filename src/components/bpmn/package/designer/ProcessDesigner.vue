<template>
  <div class="my-process-designer">
    ...
    <el-dialog :title="`预览${previewType}`"
      width="60%"
      v-model="previewModelVisible"
      append-to-body
      destroy-on-close>
      <Codemirror v-model:value="previewResult"
        :options="cmOptions"
        border
        :height="600" />
    </el-dialog>
...
const { postProcessSave } = useWorkflowApi()
export default {
  ...
  methods: {
    ...
    async handleSave() {
      ...
      let { data: fileData } = this.setEncoded('xml'.toUpperCase(), '', xml);
      let { data: svgData } = this.setEncoded("SVG", '', svg);
      const newVersion = 0
      const processId = null
      const files = [fileData, svgData]
      const params = {
        files,
        newVersion,
        processId
