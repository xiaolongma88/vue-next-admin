	...
	<div class="icon-selector w100 h100">
		...
		>
			<template #prepend>
				...
				/>
				<SvgIcon v-else-if="state.fontIconPrefix.startsWith('rk-')" :name="state.fontIconPrefix === '' ? prepend : state.fontIconPrefix" />
			...
			<template #default>
				<div class="icon-selector-warp">
					...
					<el-tabs v-model="state.fontIconTabActive" @tab-click="onIconClick">
						<el-tab-pane lazy label="svg" name="svg">
...
// 引入组件
const IconList = defineAsyncComponent(() => import('@/components/icons/src/icon-selector/list.vue'));
...
const fontIconTabNameList = () => {
	...
	else if (state.fontIconTabActive === 'svg') iconList = state.fontIconList.svg;
	return iconList;
};
// 处理 icon 双向绑定数值回显
...
const initFontIconName = () => {
	...
	else if (props.modelValue.indexOf('rk') > -1) name = 'svg';
	// 初始化 tab 高亮回显
...
const initFontIconData = async (name) => {
	if (name === 'ali') {
		...
		if (state.fontIconList.svg.length > 0) return;
		await initIconfont.svg().then((res) => {
			state.fontIconList.svg = res;
		});
	}
