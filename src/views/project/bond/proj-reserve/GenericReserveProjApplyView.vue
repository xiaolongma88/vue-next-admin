...
<template>
	<RkContainer :isFooter="false" :isAside="isAside">
		<template #header_left>
			<RkButtonGroup :options="json_jb_button" v-model="queryParams.wfStatus" @click="handleSearch" />
		</template>
		<template #header_right>
			<el-button v-if="queryParams.wfStatus == '001'" @click="handelAdd">新增项目</el-button>
			<el-button v-if="queryParams.wfStatus == '001' || queryParams.wfStatus == '004'" @click="handleOpenRecord('edit', '修改')">修改</el-button>
			<el-button v-if="queryParams.wfStatus == '001' || queryParams.wfStatus == '004'" type="danger" @click="handleOperation('delete', '删除')"
				>删除</el-button
			>
			<el-button v-if="queryParams.wfStatus == '001' || queryParams.wfStatus == '004'" type="primary" @click="handleOperation('send', '送审')"
				>送审</el-button
			>
			<el-button v-if="queryParams.wfStatus == '002'" type="danger" @click="handleOperation('cancel', '撤销送审')">撤销送审</el-button>
				...
				<template #content>
					...
					>
						<template #toolbar_title_buttons>
							<el-form-item label="单位" style="margin: 0 10px">
								<rk-select
							...
							<el-input v-model="queryParams.mhcx" placeholder="请输入申报单位/项目名称" class="mr-15px w-200px" @input="handleSearch" clearable>
							</el-input>
						</template>
					</rk-grid>
				</template>
...
import { toArray, COMMON_AMOUNT_UNIT } from '@/enums/enums-common.js';
...
const tableColumns = ref([
	...
	{ field: 'bondTypeCode', title: '申请类型', width: 120, align: 'center', formatter: ['enumsFormats', 'COMMON_BOND_TYPES'] },
...
const handleOperation = async (buttonType, textName) => {
	...
	if (buttonType == 'send') {
		...
		const deleteParams = { applyIds, taskIds, proIds, bondTypeCode: 1, nodeId: queryParams.nodeId, taskType: WF_TASK_SEND_AUDIT.value };
